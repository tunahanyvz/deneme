...
client.ayarlar = {
  official_sahip: ["465245183332581387", "414704865877557259"],
  sahip: ["465245183332581387", "414704865877557259"],
  yardimcilar: ["441169716547944448"],
  isim: "Yvz",
  dblO: "https://discordbots.org/bot/588723958153740288/vote",
  dbl: "https://discordbots.org/bot/588723958153740288",
  botD:
    "https://discordapp.com/oauth2/authorize?client_id=588723958153740288&scope=bot&permissions=2146958847",
  webS: "Yvz",
  web: "#EvdeKalTürkiye",
  versiyon: "0.0.1",
  prefix: "y!",
  ...
  secenek: ["tr", "en"],
  yenilik: ["toplu-rol-ver", "toplu-rol-al"]
};
...
const serverStats = {
  guildID: "693007803551318046",
  totalUsersID: "558352607241043990",
  memberCountID: "693007804054634516",
  botCountID: "693007804440641586"
};
  ...
  if (command === "çal") {
      ...
      try {
        ...
        try {
          ...
          msg.channel.sendEmbed(
            new Discord.RichEmbed()
              .setTitle("Yvz | Playboy Bot | Şarkı Seçimi")
              .setDescription(
                `${videos
    ...
    return msg.channel
      .sendEmbed(
        new Discord.RichEmbed()
          .setColor("RANDOM")
          .setTitle("Yvz | Playboy Şarkı Kuyruğu")
          .setDescription(
            `${serverQueue.songs
...
client.on("ready", async () => {
  ...
  client.user.setActivity(`${client.ayarlar.web}`, { type: "WATCHING" });
  let embed = new Discord.RichEmbed()
    .setTitle("** Yvz BoT Durum**")
    .setDescription(
...
client.on("message", async message => {
  ...
  if (y === 50) {
    ...
    let e = new Discord.RichEmbed()
      ...
      .setDescription(
        `Sunucu Puanı başarıyla **${y}** puana ulaştı! Premium mod aktif edildi!`
      );
  ...
  if (cmd) {
    if (db.has(`karalist_${message.author.id}`) === true) {
      let embed = new Discord.RichEmbed()
        .setColor("RANDOM")
        .setDescription(
          "Sen botun komutlarını kullanamazsın! Çünkü botun kara listesindesin!"
        );
...
client.on("message", async message => {
  ...
  if (message.content.toLowerCase().startsWith(`${prefix}talepleri-kapat`)) {
    if (!message.guild.channels.get(db.fetch(`destekK_${message.guild.id}`)))
      return;
    if (!message.guild.roles.get(db.fetch(`destekR_${message.guild.id}`)))
      return;

    if (db.has(`destekK_${message.guild.id}`) === true) {
      var kanal = message.guild.channels.get(
        db.fetch(`destekK_${message.guild.id}`)
      ).name;
      var rol = message.guild.roles.get(
        db.fetch(`destekR_${message.guild.id}`)
      );
    }
    ...
    if (
      ...
      return message.reply(
        `Bu komutu sadece ${rol} rolüne sahip kullanıcılar kullanabilir!`
      );
    ...
    const embed = new Discord.RichEmbed()
      ...
      .setDescription(
        `Tüm Destek taleplerini kapatma işlemini onaylamak için, \n10 saniye içinde \`evet\` yazınız.`
      )
      .setFooter(
        `${client.user.username} | Destek Sistemi`,
        client.user.avatarURL
    ...
    message.channel.send({ embed }).then(m => {
      message.channel
        ...
        .then(collected => {
          try {
            message.guild.channels
              .filter(c => c.name.startsWith("yardım-"))
              .forEach(async (kanal, id) => {
                kanal.delete();
              });
          } catch (e) {
            //console.log(e.stack);
        ...
        .catch(() => {
          m.edit(
            "Tüm Destek taleblerini kapatma isteği zaman aşımına uğradı."
          ).then(m2 => {
            m2.delete();
          }, 3000);
        });
...
client.on("guildCreate", guild => {
  const e = new Discord.RichEmbed()
    .setColor("RANDOM")
    .setDescription(
      "Beni Sunucuna Eklediğin İçin Teşekkür Ederim, \n Bir Ayar Komudunu Kapatmak İçin `y!ayarlar kapat` Yazarak Nasıl Kapatacağınızı Görebilirsiniz. \n Ayrıca Komutlarımı görmek için `y!yardım` yazabilirsiniz\n [DESTEK SUNUCUM](https://invite.gg/KGG7jpx) \n [WEB PANELİM](https://picayune-plantain.glitch.me)"
    )
...
client.on("messageReactionAdd", async (msgReact, user) => {
  ...
  if (
    msgReact.message.id === db.fetch(`oylamaM_${msgReact.message.guild.id}`)
  ) {
  ...
  if (db.has(`komut_${message.guild.id}`)) {
    for (var i = 0; i < db.fetch(`komut_${message.guild.id}`).length; i++) {
      ...
      if (message.content === prefix + o) {
        var a = db.fetch(`komut_${message.guild.id}`)[i][
          Object.keys(db.fetch(`komut_${message.guild.id}`)[i])
        ];
...
client.on("guildCreate", async guild => {
  ...
  var guildhook = new Discord.WebhookClient(
    "693016942021443634",
    "4MSlMxPIaOGZJjkPBF-GeHf3XowcdVndoopDhiHm4wLk4linKHB_HS9YIT4URNx0sHYu"
  );
...
client.on("message", msg => {
  ...
  if (db.has(`capsE_${msg.guild.id}`) === true) {
    const kufur = new RegExp(
      /(E|J|Q|W|R|T|Y|U|I|O|P|Ğ|Ü|A|S|D|F|G|H|K|L|Ş|İ|Z|X|C|V|B|N|M|Ö|Ç)/
    );
    if (kufur.test(msg.content) == true) {
      if (!msg.member.hasPermission("ADMINISTRATOR")) {
        msg.delete();
        ...
        var k = new Discord.RichEmbed()
          ...
          .setDescription(
            `Bu sunucuda büyük harfler **${client.user.username}** tarafından engellenmektedir!`
          );
        msg.channel.send(k).then(message => message.delete(5000));
      }
    }
...
client.on("guildMemberAdd", async member => {
  ...
  member.guild.channels
    .get(püye)
    .setName(`Üye sayısı: ${member.guild.memberCount}`);
  member.guild.channels
    .get(pbot)
    .setName(
      `Bot sayısı: ${member.guild.members.filter(m => m.user.bot).size}`
  ...
  member.guild.channels
    .get(pkanal)
    .setName(`Kanal sayısı: ${member.guild.channels.size}`);
  if (db.has(`sayac_${member.guild.id}`) === false) return;
  if (db.has(`sKanal_${member.guild.id}`) === false) return;
  const channel = db
    .fetch(`sKanal_${member.guild.id}`)
    .replace("<#", "")
    .replace(">", "");
  member.guild.channels
    .get(channel)
...
client.on("guildMemberAdd", async member => {
  ...
  const hgK = member.guild.channels.get(
    db
      .fetch(`gc_${member.guild.id}`)
      .replace("<#", "")
      .replace(">", "")
  );
  if (!hgK) return;
  ...
  hgK.send(
    db.has(`girisM_${member.guild.id}`)
      ...
      : `<@${member.user.id}> Katıldı! (\`giriş-mesaj-ayarla\` komutu ile mesaj değiştirilebilir.)`
  );
});
...
client.on("guildMemberRemove", async member => {
  ...
  hgK.send(
    db.has(`cikisM_${member.guild.id}`)
      ...
      : `**${member.user.username}** Ayrıldı! (\`çıkış-mesaj-ayarla\` komutu ile mesaj değiştirilebilir.)`
  );
});
...
client.on("roleCreate", role => {
  ...
  var kanal = role.guild.channels.get(
    db
      .fetch(`log_${role.guild.id}`)
      .replace("<#", "")
      .replace(">", "")
  );
  if (!kanal) return;
...
client.login("NTg4NzIzOTU4MTUzNzQwMjg4.XnoQIw.hdSMey-VCQDap5XSK3iGkPMlo50");
