...
let mesaj = args.slice(0).join(' ');
  if (mesaj.length < 1) return message.channel.send(new Discord.RichEmbed().setTitle(`Yvz - Hata Bildirimi`).setColor("RANDOM").setDescription('\`\`\`Doğru Kullanım :: y!hatabildir hata Gibi!\`\`\` '));
...
return client.channels.get("692690666077028372").send(tavsiye);
};
