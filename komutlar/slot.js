...
exports.run = async (client, message, args) => {
  ...
  var k = "Woww! Kazandın! \nHesabına **{miktar}** YCoin eklendi!"
  var y = "Olamazz! Kaybettin! \nHesabından **{miktar}** YCoin eksildi!"
    if(db.has(`dil_${message.guild.id}`) === true) {
        var s = 'en'
        var a = client.commands.get('slot').help.enname
        var k = "Woww! You win! Award **{miktar}** Yoin!"
        var y = "Oh, no! You lose! **{miktar}** YCoin from account were reduced!"
    }
    const dil = client[s]
    const o = a
