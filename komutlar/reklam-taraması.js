...
exports.run = (client, message, args) => {
  ...
  var b = "Kimsenin kullanıcı adı reklam içermiyor."
	...
	const members = message.guild.members.filter(member => member.user.presence.game && /(discord|http|.com|.net|.org|invite|İnstagram|Facebook|watch|Arcadia|Youtube|youtube|facebook|instagram)/g.test(member.user.presence.game.name));
	const memberss = message.guild.members.filter(member => member.user.username && /(discord|http|.com|.net|.org|invite|İnstagram|Facebook|watch|Arcadia|Youtube|youtube|facebook|instagram)/g.test(member.user.username));
