...
exports.run = function(bot, message, args) {
  ...
  message.delete(10000).catch(console.error);
    if (!message.member.voiceChannel) return message.reply("**:bangbang: Sesli kanalda değilsin. Bağlanmam için ilk önce sesli bir kanala katılmalısın. :bangbang:**").then(m => m.delete(15000)).catch(console.error);
    else {
        if (!args[0] || args[0] === "yardım" || args[0] === "yardım") {
            message.reply("**\n \n  🎵 Radyo İstasyonları 🎵 \n \n``1 = AlemFM``  \n``2 = CnnTurkFM``\n``3 = FenomenFM`` \n``4 = KralFM``\n``5 = KralPopFM``\n``6 = LineFM``\n``7 = MetroFM`` \n``8 = RadyoDFM``\n``9 = SuperFM``\n``10 = SlowTurk``\n``11 = JoyFM``\n``12 = FenomenTURK``\n``13 = SlowFm``\n``14 = TaksimClup`` \n \nÖrnek kullanım:\nAçmak için: y!radyo <Radyo Numarası> \nKapatmak için: y!radyo kapat**").then(m => m.delete(40000)).catch(console.error);
    ...
    return message.channel.send("🎧 | **Başarılı! `Radyo Kapatıldı.`.**").then(m => m.delete(10000)).catch(console.error);
        }
    }
    }
exports.conf = {
